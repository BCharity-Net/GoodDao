...
describe("Treasury contract", function () {
  ...
  beforeEach(async function () {
    ...
    goodToken = await upgrades.deployProxy(Token, [
      owner.address,
    ...
    await goodToken.setTreasuryAddress(treasury.address);
  });
  ...
  describe("Check Fees", function () {
    it("0.3 percent should be deducted on transfer from one account to another account", async function () {
      await goodToken.transfer(addr1.address, 1000);
      expect(await goodToken.balanceOf(addr1.address)).to.equal(997);
      expect(await goodToken.balanceOf(treasury.address)).to.equal(3);
    });
    it("No fees for whitelisted", async function () {
      await goodToken.setWhitelistAddress(addr1.address, true);
      await goodToken.transfer(addr1.address, 1000);
      expect(await goodToken.balanceOf(addr1.address)).to.equal(1000);
      expect(await goodToken.balanceOf(treasury.address)).to.equal(0);
    });

    it("Only owners can whitelist", async function () {
      await expect(
        goodToken.connect(addr1).setWhitelistAddress(addr1.address, true)
  ...
  describe("Fund Investee", function () {
    ...
    it("Should fund investee ", async function () {
      ...
      await goodToken.connect(owner).transfer(addr2.address, "10");
      expect(await goodToken.balanceOf(addr1.address)).to.equal(
        "13000000000000000000"
      );
      expect(
        await goodToken.balanceOf("0x000000000000000000000000000000000000dEaD")
    ...
    it("Should fund to other investee ", async function () {
      ...
      await goodToken.connect(owner).transfer(addr2.address, "10");
      await goodToken.connect(owner).transfer(addr2.address, "10");
      expect(await goodToken.balanceOf(addrs[0].address)).to.equal(
        "13000000000000000000"
      );
      expect(
        await goodToken.balanceOf("0x000000000000000000000000000000000000dEaD")
