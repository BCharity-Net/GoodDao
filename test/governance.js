...
const iGiveToken = artifacts.require("Igive");
...
describe("GovernorBravo_Propose", function () {
  ...
  beforeEach(async function () {
    ...
    this.GOOD.initialize(ownerAddress, "6666666666666666666666666666666", {
      from: ownerAddress,
      gas: 8000000,
    });
    ...
    this.iGiveToken = await iGiveToken.new({
      from: ownerAddress,
      gas: 8000000,
    });
    await this.iGiveToken.initialize(
      this.GOOD.address,
      ownerAddress,
      startBlock,
      1,
    ...
    await this.GOOD.setWhitelistAddress(ownerAddress, true, {
      from: ownerAddress,
    });
    await this.GOOD.setWhitelistAddress(userAddress1, true, {
      from: ownerAddress,
    });
    await this.GOOD.setWhitelistAddress(userAddress2, true, {
      from: ownerAddress,
    });
    await this.iGiveToken.add(100, this.GOOD.address, true, {
      from: ownerAddress,
      gas: 8000000,
    });

    await this.GOOD.approve(this.iGiveToken.address, 10000, {
      from: ownerAddress,
      gas: 8000000,
    });
    await this.GOOD.transfer(userAddress1, 10000, {
      from: ownerAddress,
      gas: 8000000,
    });
    await this.GOOD.approve(this.iGiveToken.address, 10000, {
    ...
    await this.iGiveToken.deposit(0, 1000, {
      from: ownerAddress,
      gas: 8000000,
    });
    await this.iGiveToken.deposit(0, 900, {
      from: userAddress1,
      gas: 8000000,
    });
    await this.iGiveToken.delegate(userAddress1, {
      from: userAddress1,
      gas: 8000000,
    });
    ...
    // await this.GOOD.mint(ownerAddress, "1000000000000000000000000", {from: ownerAddress, gas: 8000000})
    await this.timelock.setPendingAdmin(this.gov.address, {
      from: ownerAddress,
      gas: 8000000,
    });
    await this.gov._AcceptTimelockAdmin({ from: ownerAddress, gas: 8000000 });
    await this.GOOD.delegate(userAddress1, { from: ownerAddress });
